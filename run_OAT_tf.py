...
# Pauli operators
pauli_Z = tf.constant([[1, 0], [0, -1]], shape=(2, 2), dtype=tf.dtypes.float32)
pauli_X = tf.constant([[0, 1], [1, 0]], shape=(2, 2), dtype=tf.dtypes.float32)
...
class LindbladianMap:
    ...
    def __init__(
        ...
        *noise_data: tuple[float, tf.Tensor | tf.SparseTensor],
        ...
        self._jump_ops = [tf.math.sqrt(noise_rate) * op for noise_rate, op in noise_data if noise_rate]
    ...
    def __call__(self, density_op: tf.Tensor) -> tf.Tensor:
        ...
        if self._jump_ops:
            jump_part = tf.math.sum(op @ density_op @ tf.transpose(tf.math.conj(op)) for op in self._jump_ops)
            recycling_part = tf.math.sum(op @ density_op + density_op @ op for op in self._recycling_ops)
...
def evolve_state(
    ...
    *noise_data: tuple[float, tf.Tensor  | tf.sparse.SparseTensor],
    ...
    final_vec = tfp.math.ode.BDF(rtol=rtol,atol=atol,).solve(time_deriv, 0, density_op,
...
def simulate_OAT(num_qubits: int, params: tuple[float, float, float, float] | tf.Tensor, noise_level: float = 0) -> tf.Tensor:
    ...
    rot_axis_angle = params[3] * tf.constant(numpy.pi, dtype=tf.dtypes.float32) / 2
