...
class Dissipator:
    ...
    def __matmul__(self, density_op: tf.Tensor) -> tf.Tensor:
        density_op_size = int(tf.math.reduce_prod(density_op.shape))
        ...
        if self._rate_2:
            output += self._rate_2 * sum(self._qubit_term_2(density_op, qubit, num_qubits) for qubit in range(num_qubits))
        if self._rate_3:
            output += self._rate_3 * sum(self._qubit_term_3(density_op, qubit, num_qubits) for qubit in range(num_qubits))
        return output
...
def _qubit_term_XYZ_1(density_op: tf.Tensor, qubit: int, num_qubits: int) -> tf.Tensor:
    """
    ...
    output = tf.constant(output).reshape(input_shape)
    return output


def _qubit_term_XYZ_2(density_op: tf.Tensor, qubit: int, num_qubits: int) -> tf.Tensor:
    """
...
def _qubit_term_XYZ_3(density_op: tf.Tensor, qubit: int, num_qubits: int) -> tf.Tensor:
    """
...
def _qubit_term_PMZ_2(density_op: tf.Tensor, qubit: int, num_qubits: int) -> tf.Tensor:
    """
...
def _qubit_term_PMZ_3(density_op: tf.Tensor, qubit: int, num_qubits: int) -> tf.Tensor:
    """
