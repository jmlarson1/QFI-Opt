...
class Dissipator:
    ...
    def __init__(self, dissipation_rates: tuple[float, float, float], dissipation_format: str = "XYZ") -> None:
    ...
    def __matmul__(self, density_op: jnp.ndarray) -> jnp.ndarray:
        num_qubits = log2_int(density_op.size) // 2
        output = self._rate_1 * sum((self._qubit_term_1(density_op, qubit) for qubit in range(num_qubits)), start=jnp.array(0))
        if self._rate_2:
...
def _qubit_term_XYZ_1(density_op: jnp.ndarray, qubit: int) -> jnp.ndarray:
    """
    ...
    density_op = jnp.reshape(density_op, (dim_a, 2, dim_b, dim_a, 2, dim_b))
...
def _qubit_term_XYZ_2(density_op: jnp.ndarray, qubit: int) -> jnp.ndarray:
    """
...
def _qubit_term_XYZ_3(density_op: jnp.ndarray, qubit: int) -> jnp.ndarray:
    """
...
def _qubit_term_PMZ_2(density_op: jnp.ndarray, qubit: int) -> jnp.ndarray:
    """
...
def _qubit_term_PMZ_3(density_op: jnp.ndarray, qubit: int) -> jnp.ndarray:
    """
