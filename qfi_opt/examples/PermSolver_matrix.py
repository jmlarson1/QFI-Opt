...
def DisMat(gamma_p, gamma_m, gamma_z, gamma_pall, gamma_mall,gamma_zall, N):
    #table for j, mj
    #print(gamma_p, gamma_m, gamma_z, gamma_pall, gamma_mall,gamma_zall)
    tablej=[]
    tablemj1=[]
    tablemj2=[]
    ...
    def Fgamma1(tablej,tablemj1,tablemj2,N):
        gamma1=0.5*gamma_mall*((tablej+tablemj1)*(tablej-tablemj1+1)+(tablej+tablemj2)*(tablej-tablemj2+1))
        gamma1+=0.5*gamma_pall*((tablej-tablemj1)*(tablej+tablemj1+1)+(tablej-tablemj2)*(tablej+tablemj2+1))
    ...
    def Fgamma2(tablej,tablemj1,tablemj2,N):
        gamma2=gamma_mall*np.sqrt((tablej+tablemj1)*(tablej-tablemj1+1)*(tablej+tablemj2)*(tablej-tablemj2+1))
        gamma2+=0.5*gamma_m*np.sqrt((tablej+tablemj1)*(tablej-tablemj1+1)*(tablej+tablemj2)*(tablej-tablemj2+1))*(N+1)/tablej/(tablej+1)
        return gamma2
        
    def Fgamma8(tablej,tablemj1,tablemj2,N):
        gamma8=gamma_pall*np.sqrt((tablej-tablemj1)*(tablej+tablemj1+1)*(tablej-tablemj2)*(tablej+tablemj2+1))
        gamma8+=0.5*gamma_p*np.sqrt((tablej-tablemj1)*(tablej+tablemj1+1)*(tablej-tablemj2)*(tablej+tablemj2+1))*(N+1)/tablej/(tablej+1)
        return gamma8
    ...
    def Fgamma3(tablej,tablemj1,tablemj2,N):
        gamma3=0.5*gamma_m*np.sqrt((tablej+tablemj1)*(tablej+tablemj1-1)*(tablej+tablemj2)*(tablej+tablemj2-1))*(N+tablej+1)/tablej/(2*tablej+1)
        return gamma3
        
    def Fgamma7(tablej,tablemj1,tablemj2,N):
        gamma7=0.5*gamma_p*np.sqrt((tablej-tablemj1)*(tablej-tablemj1-1)*(tablej-tablemj2)*(tablej-tablemj2-1))*(N+tablej+1)/tablej/(2*tablej+1)
        return gamma7
    ...
    def Fgamma4(tablej,tablemj1,tablemj2,N):
        gamma4=0.5*gamma_m*np.sqrt((tablej-tablemj1+1)*(tablej-tablemj1+2)*(tablej-tablemj2+1)*(tablej-tablemj2+2))*(N-tablej)/(tablej+1)/(2*tablej+1)
        return gamma4
                        
    def Fgamma9(tablej,tablemj1,tablemj2,N):
        gamma9=0.5*gamma_p*np.sqrt((tablej+tablemj1+1)*(tablej+tablemj1+2)*(tablej+tablemj2+1)*(tablej+tablemj2+2))*(N-tablej)/(tablej+1)/(2*tablej+1)
        return gamma9
...
# isotropic dissipation matrix
def isotropic_DisMat(gamma_x, gamma_y, gamma_z, N):
    # table for j, mj
    # print(gamma_p, gamma_m, gamma_z, gamma_pall, gamma_mall,gamma_zall)
    tablej = []
    tablemj1 = []
    ...
    def Fgamma2(tablej, tablemj1, tablemj2, N):
        gamma2 = 0.5 * (gamma_x + gamma_y) / 4. * np.sqrt(
            (tablej + tablemj1) * (tablej - tablemj1 + 1) * (tablej + tablemj2) * (tablej - tablemj2 + 1)) * (
                             N + 1) / tablej / (tablej + 1)
        return gamma2

    def Fgamma8(tablej, tablemj1, tablemj2, N):
        gamma8 = 0.5 * (gamma_x + gamma_y) / 4. * np.sqrt(
            (tablej - tablemj1) * (tablej + tablemj1 + 1) * (tablej - tablemj2) * (tablej + tablemj2 + 1)) * (
                             N + 1) / tablej / (tablej + 1)
        return gamma8
    ...
    def Fgamma3(tablej, tablemj1, tablemj2, N):
        gamma3 = 0.5 * (gamma_x + gamma_y) / 4. * np.sqrt(
            (tablej + tablemj1) * (tablej + tablemj1 - 1) * (tablej + tablemj2) * (tablej + tablemj2 - 1)) * (
                             N + tablej + 1) / tablej / (2 * tablej + 1)
        return gamma3

    def Fgamma7(tablej, tablemj1, tablemj2, N):
        gamma7 = 0.5 * (gamma_x + gamma_y) / 4. * np.sqrt(
            (tablej - tablemj1) * (tablej - tablemj1 - 1) * (tablej - tablemj2) * (tablej - tablemj2 - 1)) * (
                             N + tablej + 1) / tablej / (2 * tablej + 1)
        return gamma7
...
def OATMat(chi, N): # H=\chi Sz^2
    ...
    for i in range(0,dimension):
        ...
        for j in range(0,dimension):
            ...
            if(i==j):
                gamma0=chi*(tablemj2[i]**2-tablemj1[i]**2)*1j
                if(abs(gamma0)>=1e-10):
...
def TATMat(chi, N): #H=\chi (SxSy+SySx) #Possible issue with missing terms?
    #table for j, mj
    tablej=[]
    tablemj1=[]
    tablemj2=[]
            ...
            #p(j,m,m'-2)
                if(tablemj1[i]==tablemj1[j] and tablemj2[i]-2==tablemj2[j]):
                    ...
                    if(abs(gamma0)>=1e-10):
                        resultmat[i].append(gamma0/2)
                        resultmatloc[i].append(j)
             #Note the possibility of missing terms/overcounting? Test half and all case, compare if the result same
             #p(j,m,m'+2)
                if(tablemj1[i]==tablemj1[j] and tablemj2[i]+2==tablemj2[j]):
                    gamma0=0.5*chi*np.sqrt((tablej[i]*(tablej[i]+1)-(tablemj2[i]+2)*(tablemj2[i]+1))*(tablej[i]*(tablej[i]+1)-(tablemj2[i]+1)*tablemj2[i]))
                    if(abs(gamma0)>=1e-10):
...
def LMGMat2(chi, Omega, N): #-chi/(2N)*Sz^2-Omega*S_x
    ...
    for i in range(0,dimension):
        ...
        for j in range(0,dimension):
            if(i==j):
            #p(j,m,m')
                gamma0=1j*chi*(tablemj1[i]**2-tablemj2[i]**2)
                if(abs(gamma0)>=1e-10):
                    resultmat[i].append(gamma0)
                    resultmatloc[i].append(j)
            elif(tablej[i]==tablej[j]):
            #p(j,m-1,m')
                if(tablemj1[i]-1==tablemj1[j] and tablemj2[i]==tablemj2[j]):
                    gamma0=1j*Omega*np.sqrt(tablej[i]*(tablej[i]+1)-(tablemj1[i]-1)*tablemj1[i])
                    if(abs(gamma0)>=1e-10):
                        resultmat[i].append(gamma0)
                        resultmatloc[i].append(j)
            #p(j,m+1,m')
            ...
            #p(j,m,m'-1)
                if(tablemj1[i]==tablemj1[j] and tablemj2[i]-1==tablemj2[j]):
                    gamma0=-1j*Omega*np.sqrt(tablej[i]*(tablej[i]+1)-(tablemj2[i]-1)*tablemj2[i])
                    if(abs(gamma0)>=1e-10):
                        resultmat[i].append(gamma0)
                        resultmatloc[i].append(j)
             #Note the possibility of missing terms/overcounting? Test half and all case, compare if the result same
