...
def initial_step_size(fun, t0, y0, order, rtol, atol, f0):
    ...
    h1 = jnp.where((d1 <= 1e-15) & (d2 <= 1e-15), jnp.maximum(1e-6, h0 * 1e-3), (0.01 / jnp.maximum(d1, d2)) ** (1.0 / (order + 1.0)))
...
def runge_kutta_step(func, y0, f0, t0, dt):
    ...
    c_error = jnp.array(
        [35 / 384 - 1951 / 21600, 0, 500 / 1113 - 22642 / 50085, 125 / 192 - 451 / 720, -2187 / 6784 - -12231 / 42400, 11 / 84 - 649 / 6300, -1.0 / 60.0],
        dtype=f0.dtype,
    )
...
def _odeint(func, rtol, atol, mxstep, hmax, y0, ts, *args):
    ...
    def scan_fun(carry, target_t):
        ...
        def body_fun(state):
            ...
            dt = jnp.clip(optimal_step_size(dt, error_ratio), a_min=0.0, a_max=hmax)
    ...
    f0 = func_(y0, ts[0])
    dt = jnp.clip(initial_step_size(func_, ts[0], y0, 4, rtol, atol, f0), a_min=0.0, a_max=hmax)
...
def _odeint_rev(func, rtol, atol, mxstep, hmax, res, g):
    ...
    def scan_fun(carry, i):
        ...
        _, y_bar, t0_bar, args_bar = odeint(
            aug_dynamics, (ys[i], y_bar, t0_bar, args_bar), jnp.array([-ts[i], -ts[i - 1]]), *args, rtol=rtol, atol=atol, mxstep=mxstep, hmax=hmax
        )
