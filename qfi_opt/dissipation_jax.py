...
class Dissipator:
    ...
    def __matmul__(self, density_op: np.ndarray) -> np.ndarray:
        ...
        output = sum((rate * term(density_op, qubit) for rate, term in zip(self._rates, self._terms) for qubit in range(num_qubits)))
        return output if issubclass(type(output), type(density_op)) else 0 * density_op
